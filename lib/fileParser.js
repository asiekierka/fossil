...
var runList = function(dirs, extensions, fn) {
	_.each(dirs, function(dir) {
		_.each(util.onlyExtension(wrench.readdirSyncRecursive(dir), extensions), function(name) {
			util.say("info", "Parsing " + path.basename(name));
			fn(dir, name);
		});
	});
}

function extendModInfo(_filename, info) {
	var filename = path.basename(_filename);
	info.filename = filename;
	info.location = _filename;
...
exports.getModList = function(directories) {
	var mods = [];
	runList(directories, [".zip", ".jar", ".litemod"], function(dir, name) {
...
exports.getPluginList = function(directories) {
	var plugins = [];
	runList(directories, [".jar"], function(dir, name) {
